...
>title="自訂元件"
>abstract="CCOM會識別AEM上安裝的自訂元件。 提供這項資訊是為了最佳實務評估目的。"

`CCOM` 識別AEM上安裝的自訂元件。 提供這項資訊是為了最佳實務評估目的。

此程式碼使用子類型來識別元件類別：

* `custom.core`：元件的超級型別鏈結中的某個超級型別包含 `core/wcm/components/`，表示繼承自核心元件。
* `custom.foundation`：元件的超級型別鏈結中的某個超級型別包含&quot;`core/wcm/components/`，表示繼承自核心元件。
* `custom.overlay.foundation`：元件路徑包含 `wcm/foundation/components/` 或 `foundation/components/`，表示其覆蓋基礎元件。
