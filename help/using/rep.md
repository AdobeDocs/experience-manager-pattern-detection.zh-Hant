...
>title="複寫代理"
>abstract="REP 會識別已啟用的複寫代理。由於在升級為 AEM as a Cloud Service 時可能存在應解決的問題，因此報告了這些複寫代理。AEM as a Cloud Service 使用 Sling 內容發佈，將內容從製作環境散發到發佈環境。這是在AEM執行階段之外使用Adobe Developer上Adobe I/O Runtime的管道服務完成的。 這是在布建的AEMas a Cloud Service環境中自動設定的。"
...
AEM as a Cloud Service 使用 [Sling 內容發佈](https://sling.apache.org/documentation/bundles/content-distribution.html)，將內容從製作環境散發到發佈環境。這是在AEM執行階段之外使用Adobe Developer上Adobe I/O Runtime的管道服務完成的。 這是在布建的AEMas a Cloud Service環境中自動設定的。
